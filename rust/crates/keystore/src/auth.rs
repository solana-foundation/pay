...
impl AuthIntent {
    ...
    pub fn authorize_payment_details(amount: &str, reason: &str, operator: &str) -> Self {
        Self::AuthorizePayment {
            message: payment_details_message(amount, reason, operator),
            limit: PaymentLimit::from_amount(amount),
        }
    }
    ...
    pub fn payment_limit(&self) -> Option<PaymentLimit> {
        match self {
            Self::AuthorizePayment { limit, .. } | Self::AuthorizeSubscription { limit, .. } => {
                *limit
            }
            _ => None,
        }
...
fn payment_details_message(amount: &str, reason: &str, operator: &str) -> String {
