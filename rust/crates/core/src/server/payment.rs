...
pub async fn payment_middleware<S: PaymentState>(
    ...
    if let Some(sub_spec) = subscription_config {
        ...
        let operator_rpc_url = mpps
            ...
            .unwrap_or_else(|| {
                solana_mpp::protocol::solana::default_rpc_url("mainnet-beta").to_string()
            });
...
async fn handle_subscription_endpoint(
    ...
    let defaults = crate::server::subscription::OperatorDefaults {
        ...
        secret_key: None,
        realm: Some(subdomain),
        fee_payer,
        fee_payer_signer: fee_payer_signer.clone(),
    };
    let server = match crate::server::subscription::build_handler(spec, defaults, endpoint_description) {
        Ok(s) => s,
        Err(e) => {
            return (
                StatusCode::INTERNAL_SERVER_ERROR,
    ...
    match server.verify_credential(&credential).await {
        ...
        Err(e) => {
                ...
                && let Ok(v) = axum::http::HeaderValue::from_str(&www_auth)
            {
                response
                    .headers_mut()
                    .insert(WWW_AUTHENTICATE_HEADER, v);
            }
            response
        }
    }
...
fn subscription_challenge_response(
    ...
    let challenge = match server_subscription::build_challenge(spec, defaults, endpoint_description) {
        Ok(c) => c,
        Err(e) => {
    ...
    let www_auth = match solana_mpp::format_www_authenticate(&challenge) {
        Ok(h) => h,
        Err(e) => {
